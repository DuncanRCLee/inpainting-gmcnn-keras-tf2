...
class Dataset:
    def __init__(self, train_path, test_path, batch_size, img_height, img_width):
        data_generator = tf.keras.preprocessing.image.ImageDataGenerator(preprocessing_function=NORM_MINUS_PLUS_ONE)
...
class MaskDataset:
    def __init__(self, train_path, batch_size, img_height, img_width):
        data_generator = tf.keras.preprocessing.image.ImageDataGenerator(preprocessing_function=NORM_MASK)
        self._train_set = data_generator.flow_from_directory(train_path,
                                                             target_size=(img_height, img_width),
                                                             batch_size=batch_size,
                                                             class_mode=None)
